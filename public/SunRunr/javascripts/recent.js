function getRecentPositions() {
  $.ajax({
    url: '/position/recent/3',
    type: 'GET',
    headers: { 'x-auth': window.localStorage.getItem("authToken") },
    dataType: 'json'
  })
    .done(displayMostRecentPosition)
    .fail(recentPositionError);
}

function displayMostRecentPosition(data, textSatus, jqXHR) {
   $("#main").show();

   // If there's at least one pothole, draw the map
   let latitude = 32.2319;
   let longitude = -110.9501;
   let positionReport = "No position have been reported in the last three days.";
   
   if (data.positions.length > 0) {
	   let latitude = data.positions[data.positions.length-1].latitude;
	   let longitude = data.positions[data.positions.length-1].longitude;
		
      // Add descriptive text of the pothole recently reported 
      positionReport = data.positions.length //+
	                 " positions have been reported in the last three days."// The most recent position (shown above) was hit " +
	                  //data.potholes[data.potholes.length-1].totalHits + " times.";
   }
	    
   $("#positionText").html(positionReport);
   // Create a map centered at the most recent pothole location
   ResultH = []
   for (let position of data.poaitions) {
      uluru = {lat: position.latitude, lng: position.longitude};
      ResultH.push(uluru);
   }     
  $("#positionText").html(ResultH);
               
}

function recentPositionError(jqXHR, textStatus, errorThrown) {
  ...
  if( jqXHR.status === 401 ) {
    ...
    $("#positionText").html("Error: " + status.message);
    $("#positionText").show();
  } 
}
...
function initRecent() {
    ...
    $("#refreshRecent").click(getRecentPositions);
    getRecentPositions();
}
